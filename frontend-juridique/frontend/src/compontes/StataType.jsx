...
function StatType() {
  ...
  const drawChartAllDates = () => {
    ...
    const labels = affairesList.map(affaire => affaire.type);
    ...
    setChartOptions({
      ...
      title: {
        text: 'Comparaison des nombres d\'affaires pour chaque type',
        align: 'center',
        style: {
          fontSize: '18px',
          fontFamily: 'Arial',
      ...
      colors: randomColors(dataSets.length),
    });

    setChartSeries([{
      name: 'Nombre d\'affaires par type',
      data: dataSets,
    }]);
  };
  ...
  const getAffairesList = (affairesData) => {
    ...
    affairesData.forEach(affaire => {
      const type = affaire.typeAffaire;
    ...
    return Array.from(typesMap, ([type, nombres]) => ({ type, nombres }));
  };
  ...
  return (
    <Layout>
      <div className="stat-container">
        <h2>Comparaison des nombres d'affaires pour chaque type</h2>
        <form>
